(function () {
    ...
    const groupLink = qs("#groupOrderLink");
    ...
    const setGroupLink = (productId) => {
        if (!groupLink) return;
        if (productId > 0) {
            groupLink.href = `product-order-group.html?product_id=${encodeURIComponent(String(productId))}`;
        }
    };
